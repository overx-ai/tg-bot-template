...
class SupportBot:
    ...
    async def _forward_to_support(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            await message.reply_text(
                "Sorry, we couldn't send your message to support. Please try again later."
            )
    ...
    async def _handle_admin_reply(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not message.reply_to_message:
            await message.reply_text(
                self.locale_manager.get("support_use_reply")
            )
            return
        ...
        try:
            ...
            if "👤 **User:** " in original_text:
                ...
                reply_text = self.locale_manager.format(
                    "support_reply",
                    answer=message.text
                )
                ...
                await message.reply_text(
                    "❌ Could not extract user ID from the original message."
                )
        
        except (ValueError, IndexError) as e:
            logger.error(f"Error parsing user ID: {e}")
            await message.reply_text(
                "❌ Could not parse user ID. Please check the message format."
            )
